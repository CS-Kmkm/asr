...
- NVIDIA CUDA-capable GPU with 12 GB VRAM or more — **required only for the
  VibeVoice backend**; the faster-whisper backend runs on CPU and needs no GPU
...
Select **Load VibeVoice** before the first dictation. The worker downloads
`microsoft/VibeVoice-ASR-HF` through Transformers/Hugging Face on first load and
uses the normal Hugging Face cache. The model is not bundled with the app.
...
### faster-whisper (CPU-capable)

A lightweight backend that runs on CPU or CUDA. No NVIDIA GPU required.
...
```powershell
$env:ASR_WORKER_BACKEND = "faster-whisper"
# or pass --backend faster-whisper to the worker directly
```
...
### VibeVoice (GPU required, high-quality option)

Uses `microsoft/VibeVoice-ASR-HF` via Transformers. Requires a CUDA GPU.
CPU fallback is intentionally disabled. This is the default backend
(`ASR_WORKER_BACKEND=vibevoice`).
