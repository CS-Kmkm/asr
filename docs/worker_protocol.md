...
- 1リクエストにつき必ず1レスポンスが返る、同期的な request/response 往復モデルである
  (ADR-0002 のbatch方式に対応)。ただし `load` に限り、レスポンスの前に同一 `id` の
  **進捗通知行**(`event: "progress"`、`ok` を持たない)が0行以上先行しうる(§2.2.1)。
...
#### 2.2.1 load の進捗通知(実装済み)

`load` のレスポンスの前に、同一 `id` を持つ **進捗通知行**が0行以上出力される。通知行は
`ok` も `error` も持たず、`event: "progress"` で識別する。デスクトップ側は「キャッシュ済み
モデルの読み込み」と「初回のモデルダウンロード」を別のメッセージとして表示するため、
どちらを実行しているかはワーカーだけが判断できる。

**通知はクライアントのオプトインが必須である。** ワーカーは環境変数
`ASR_WORKER_PROGRESS=1` が設定されている場合にのみ通知行を出力し、未設定なら
v1 と完全に同一の「1リクエスト1レスポンス」で応答する。Rust側は `WorkerCommand::python`
でこの環境変数を常に付与する。ワーカーはリポジトリのソースから起動される一方
デスクトップ実行ファイルは別途ビルドされるため(ADR-0003)、通知を知らない旧ビルドが
新しいワーカーを起動する組み合わせが実際に起こりうる。その場合、旧クライアントは通知行を
レスポンスとして検証し `invalid type: null, expected struct WorkerError` で失敗する
(`docs/incidents/2026-09-06-model-preparation-message.md`)。

| フィールド | 型 | 必須 | 説明 |
|---|---|---|---|
| `id` | リクエストと同一 | 必須 | 対象リクエストのエコーバック。 |
| `event` | `"progress"` | 必須 | 通知行であることの識別子。 |
| `stage` | `"download"` / `"load"` | 必須 | `download` はモデルファイル取得中、`load` はローカルのファイルをメモリへ読み込む段階。 |
| `model` | 文字列 | 任意 | ダウンロード対象のHugging Faceリポジトリ。 |
| `completed_bytes` | 整数 | 任意 | ダウンロード済みバイト数(全ファイルの合計)。 |
| `total_bytes` | 整数 / `null` | 任意 | 総バイト数。Hugging Faceがファイルメタデータを返すまでは不明であり `null` になる。 |

```json
→ {"id": 1, "command": "load", "quantization": "4bit"}
← {"id": 1, "event": "progress", "stage": "download", "model": "Systran/faster-whisper-tiny"}
← {"id": 1, "event": "progress", "stage": "download", "model": "Systran/faster-whisper-tiny", "completed_bytes": 2665349, "total_bytes": 75538270}
← {"id": 1, "event": "progress", "stage": "load"}
← {"id": 1, "ok": true, "model": "faster-whisper:tiny", "quantization": "4bit"}
```
...
- VibeVoice: transformers の `from_pretrained` に進捗フックがないため、未キャッシュ時に
  バイト数なしの `download` を1回だけ通知する。
- openai-compatible / mock: ダウンロードを伴わないため通知しない。
...
- タイムアウトは **1行の受信ごと**に適用される。したがって進捗通知(§2.2.1)を受信すると
  待機時間はリセットされる。
...
- `exchange` は `ok` を持たず `event: "progress"` かつ `id` が一致する行を進捗通知として
  取り込み、次の行の読み取りを続ける。それ以外の行は通常のレスポンス検証にかけるため、
  `id` 不一致や不正な通知はプロトコルエラー(=リセット)になる。
- 取り込んだ通知は `tokio::sync::broadcast`(容量32)へ流す。進捗は補助情報であり、
  受信側の遅延でロード本体を待たせない。購読者がいない場合や取りこぼしは無視される。
- `Transcriber::load_progress` は購読用レシーバを返す。Rust側はモデルロード中のみ購読し、
  最初の `download` 通知でステータスメッセージをダウンロード表示へ切り替え、
  各通知を `model-progress` イベントとしてUIへ転送する。
...
>
> 例外として、進捗イベントのうち **`load` の進捗通知のみ実装済み**であり、仕様は §2.2.1 に
> ある。本節 5.2 が扱う `transcribe` の進捗は引き続き未実装である。
...
- クライアントは `ok` も `error` も持たない行を中間イベントとして扱い、最終行で完了とみなす。
  この読み取り規約は `load` の進捗通知(§2.2.1)としてすでに実装済みであり、`transcribe` へ
  拡張する場合も同じ形式を用いる。
