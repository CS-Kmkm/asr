...
- Follow-up integration completed on 2026-09-13: `main` was fast-forwarded to the consolidated HEAD and published. GitHub Actions run `34758566698` passed its frontend, Python worker, and Rust jobs.
- `fix/correction-insertion` and `fix/issue-2-color-semantics` were not ancestry-merged because their implementation patches were already represented by the reviewed conflict-resolved commits. Re-merging them would have duplicated obsolete variants. After remote `main` matched the consolidated HEAD and CI passed, both fix branches and `integration/unified-history` were deleted together with their clean auxiliary worktrees.
- Issue #1 is resolved by the selected-text translation command/hotkey, direction detection, persisted settings, and target/clipboard safety paths. Issue #2 is resolved by the light accessible palette, semantic notice severities, unified CSS token source, and disabled-hover fix. Issue #3 is resolved by persisted Japanese/English UI selection and localized static and provisional-insertion status notices.
- Remaining: none. `.diagnostics/` remains intentionally untracked user runtime output.
